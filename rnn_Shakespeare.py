...
cell_1 = tf.nn.rnn_cell.BasicRNNCell(num_units=n_neurons_1,activation=tf.nn.tanh,name="cell_1")
cell_2 = tf.nn.rnn_cell.BasicRNNCell(num_units=n_neurons_2,activation=tf.nn.tanh,name="cell_2")
multi_layer_cell = tf.nn.rnn_cell.MultiRNNCell([cell_1,cell_2])
states, fin_state = tf.nn.dynamic_rnn(multi_layer_cell,X,dtype=tf.float32) #states: None*seq_length*n_neurons_2, fin_state: None*n_neurons_2
outputs = tf.layers.dense(states,n_outputs,name="dense") #None*seq_length*n_outputs
